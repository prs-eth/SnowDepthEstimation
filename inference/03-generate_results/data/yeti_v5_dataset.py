...
def get_random_window(img_size = [34858, 22075], offset=[0, 0], patch_size = 256):
    ...
    Returns:
        [type]: [description]
    """
    # x1 = randint(offset[0], img_size[0] - patch_size) # left boundary
    x1 = torch.randint(offset[0], img_size[0] - patch_size, (1,)) # left boundary
    x2 = x1 + patch_size # right boundary + 1
    # y1 = randint(offset[1], img_size[1] - patch_size) # top boundary
...
# def window_check(w, days, base_path):
#     mask_path = os.path.join(base_path, 'mask', 'mask_LV95.tif')
#     mask_max = np.max(rasterio.open(mask_path).read(window=w))
#     if mask_max == 0:
#         return False
    
#     path = os.path.join(base_path, 'snow_depth', str(days[-1]), 'snow_depth.tif')
#     depth_max = np.max(rasterio.open(path).read(window=w))
#     if depth_max == 0:
#         return False
...
class YetiV5Dataset(BaseDataset):
...
#         parser.set_defaults(max_dataset_size=10, new_dataset_option=2.0)  # specify dataset-specific default values
    ...
    def __getitem__(self, index):
        ...
        while not found:
            ...
            square_idx = torch.randint(self.mask.shape[0], (1,))
            ...
            # print('\n\n\nFIX DATALOADER HERE BEFORE RUNNING\n\n\n')
            # found = True
            ...
            if self.d_ch == 4:
                dynamic_inputs = dynamic_inputs[:, :4, :, :]
                found = True
            else:
                found = dynamic_inputs[:,6,:,:].std() > 0.001

            cc += 1
            if cc > 50:
                print('Warning: S2 image seems to be black')
                found = True
        ...
        if self.no_dem:
            static_inputs = torch.zeros((0, h2-h1, w2-w1))
        else:
        ...
        if self.benchmark:
            ...
            #     if 'date_list' not in k:
            #         print('{}.shape = {}'.format(k, sample[k].shape))
            # print('\n')
