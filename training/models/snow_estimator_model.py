...
import datetime
import copy
from util.probabilistic_losses import GaussianNLLLoss
...
class SnowEstimatorModel(BaseModel):
    """
    ...
    def modify_commandline_options(parser):
        ...
        parser.add_argument('--n_days', type=int, default=26, help='Number of days considered in each sequence.')
        ...
        parser.add_argument('--in_ch_dynamic', type=int, default=16, help='Number of days considered in each sequence. (S2=12, S2_PRB=2, S1=4)')
        parser.add_argument('--in_ch_static', type=int, default=6, help='Number of days considered in each sequence.')
        parser.add_argument('--rnn_type', type=str, default='ConvGRU')

        parser.add_argument('--ignore_first_N', type=int, default=1, help='Number of days ignored for loss in the beginning of sequence.')
    ...
    def __init__(self, opt):
        ...
        self.visual_names = ['I_in', 'depth_gt', 'depth_pred']
        ...
        for p_m in self.netModel.parameters():
        ...
        for i in range(self.dynamic_inputs.shape[1]):
            ...
            if validation:
                if self.hidden is not None:
                    self.hidden_fake = [0*x for x in self.hidden]
                    self.hidden = self.netModel(self.inputs, hidden=self.hidden_fake)
                else:
                ...
                if self.hidden is not None:
                    self.hidden_fake = [0*x for x in self.hidden]
                    self.hidden = self.netModel_NoMom(self.inputs, hidden=self.hidden_fake)
                else:
